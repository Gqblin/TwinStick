...
public class EnemyFOV : MonoBehaviour
{
    ...
    private void Update()
    {
        if (visibleTargets.Count == 0 && attackedByPlayer != true)
        {
            enemyState = EnemyState.walking;
        }
        else if (visibleTargets.Count != 0 || attackedByPlayer == true)
        {
            enemyState = EnemyState.following;
            attackedByPlayer = true;
        }
        if (visibleTargets.Count != 0 && enemyState == EnemyState.following)
        {
            Enemies enemy = GetComponent<Enemies>();
            enemy.TurnToPlayer();
        }
        else if (visibleTargets.Count == 0 && enemyState == EnemyState.following)
        {
            return;
        }
    }
    ...
    void FindVisibleTargets()
    {
        ...
        Collider[] targetsInViewRadius = Physics.OverlapSphere(transform.position, viewRadius, playerTargetMask);
    ...
    void DrawVieldOfView()
    {
        int stepCount = Mathf.RoundToInt(viewAngle * meshResolution);
        float stepSizeAngle = viewAngle / stepCount;
        List<Vector3> viewPoints = new List<Vector3>();
        ViewCastInfo oldViewCast = new ViewCastInfo();

        for (int i = 0; i < stepCount; i++)
        {
            float angle = transform.eulerAngles.y - viewAngle / 2 + stepSizeAngle * i;
    ...
    {
        Vector3 dir = DirFromAngle(globalAngel, true);
        RaycastHit hit;
